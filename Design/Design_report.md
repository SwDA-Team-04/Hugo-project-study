...
This hub is used by ```refreshPathForPort``` method (```livereload/livereload.go```) to send messages in broadcast. The hub distibutes the message to all the registered connections. This allows the browsers, connected throught web socket connections, to receive che message and to react accordingly.
